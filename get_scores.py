...
class Predictions:
    ...
    def get_scores(self, start_data, output_file_name, output_debug_file_name, speed_run, debug, tqdm_colour, text_colour, reloading):
        ...
        sequencelib.add_data('Theophylline', '6', start_data, structure_mode='from_ct', top_k=None)
        with open(f'results/{TIME}/sequencelib.pickle', 'wb') as f:
            pickle.dump(sequencelib, f)
            f.close()
        get_priority_clusters()
        data = generate_all_recommendations(['Theophylline'], [6], TRAIN_PERCENT)
